# app.py — Streamlit deployment (fixed preprocessing, class mapping & scaling)
...
# Load class names saved at training time (fallback to default)
if os.path.exists(CLASS_NAMES_FILE):
    with open(CLASS_NAMES_FILE, 'r') as f:
        CLASS_NAMES = json.load(f)
else:
    CLASS_NAMES = ['bird', 'drone']  # fallback - keep same order as training
...
MODELS=load_models()
# Utility: does model include rescaling
def model_has_rescaling(model):
    ...
    for layer in model.layers:
        if isinstance(layer, Rescaling):
            return True
        # If model is a Functional model and contains nested models, check their layers too
        # (e.g., a base model inside a wrapper)
...
def predict_image_with_model(pil_image: Image.Image, model):
    ...
    # If model does NOT include a Rescaling layer, scale here.
